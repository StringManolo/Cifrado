function myszkowski(texto, clave) {
  // Eliminamos todos los espacios del texto
  // texto = texto.replace(/\s/g, '');
  // Obtenemos la longitud de la clave y del texto
  const longitud_clave = clave.length;
  ...
  for (let i = 0; i < longitud_clave; i++) {
    matriz[i] = new Array(longitud_texto);
  }
...
function desmyszkowski(texto_cifrado, clave) {
  // Obtenemos la longitud de la clave y del texto cifrado
  const longitud_clave = clave.length;
  ...
  for (let i = 0; i < longitud_clave; i++) {
    ...
    for (let j = 0; j < letras_columna; j++) {
      matriz[columna][j] = texto_cifrado.charAt(indice);
      indice++;
    }
  }
