...
let decipher = crypto.createDecipheriv('ChaCha20-Poly1305', deriveKeyFromText(key), nonce, { authTagLength: 16 });                                                              let decryptedChacha = decipher.update(encryptedChacha, 'hex', 'utf8');
...
const beat = myszkowski(vernam(encryptedChacha, key), key);                             // reuse decipher
...
// debug only                                                                           revTag = revTag.replace(/[0-9]/g, 7);
*/
...
Chacha20-Poly1305 + Vernam + Myszkowski: ${beat}                                        Descifrado: ${revBeat}

Chacha20-Poly1305 + Vernam + Myszkowski (Including AuthTag and IV): ${aibeat}
Descifrado: ${revAibeat}
El nonce utilizado es: ${revIV.toString('hex')}                                         El tag de Poly1305 es: ${revTag}
